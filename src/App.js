...
function App() {
  ...
  const mostrarFichaFunc = (persona) => {
  ...
  const handleMostrarFechaHora = () => {
    setMostrarFechaHora(true);
    setShowFicha(false);
    setPeople({ axios: [], fetch: [] });
    setIsLoading(false);
    setLoadingType('');
  ...
  return (
    <div className="App">
      ...
      <div className="App-controls">
        ...
        <button onClick={handleMostrarFechaHora} disabled={isLoading} className="btn">
      ...
      {mostrarFechaHora ? (
        ...
        <FichaPersonal persona={personaSeleccionada} volver={() => setShowFicha(false)} />
      ) : (
        <>
                ...
                {people.axios.length > 0 ? (
                  people.axios.map(person => (
                    <div key={person.login.uuid}>
                      ...
                      <button className="btn" onClick={() => mostrarFichaFunc(person)}>Ver ficha</button>
                    </div>
                  ))
                ) : (
